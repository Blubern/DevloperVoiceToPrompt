...
export function addEnhancerTemplate(
  name: string,
  text: string
...
export function updateEnhancerTemplate(
  id: string,
  name: string,
  text: string
): Promise<void> {
...
export function deleteEnhancerTemplate(id: string): Promise<void> {
  return serialized(async () => {
    const s = await getStore();
    const entries = await getEnhancerTemplates();
...
export function resetEnhancerTemplates(): Promise<EnhancerTemplate[]> {
  return serialized(async () => {
    const s = await getStore();
    const now = new Date().toISOString();
    const defaults: EnhancerTemplate[] = DEFAULT_ENHANCER_TEMPLATES.map((t) => ({
